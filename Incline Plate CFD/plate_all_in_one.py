...
=======================================================
Everything (Part A empirical analysis + Part B lattice Boltzmann CFD:
solver, cylinder validation, and the 45-degree plate run) is in this one
file, so there is nothing to import. Paste it all into a single notebook
cell and run, or:  python plate_all_in_one.py

Plots are shown inline (in a notebook) and also saved to ./figures/.

  QUICK = True   -> coarse grids / few steps, ~1 minute  (sanity check)
  QUICK = False  -> full resolution, ~6-8 minutes        (report figures)
...
QUICK = True     # <-- set to False for full-resolution report figures
